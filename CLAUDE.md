...
**进度 127 / 139。** 回归读数：`uitest` **106/106** · `selftest` 零 error · `lifecycletest` 全通 ·
...
1. ⚠️ **请用户跑一次 `/design-login`** —— MCP 的 design scopes 掉了，第九轮回执上传不了。
   回执全文已在 `doc/14` 的「第九轮回执」一节，恢复后放成它的 `uploads/36-…`
2. **等设计侧定一件**：⌘E 在稿里够不着（稿在 iframe，焦点进去 keydown 到不了我们的 document）。
   三个方案在回执里，要改的是它的 `S2-单稿预览壳.dc.html`
