...
**线 A · 验证两条 AI 通道**（`doc/12` M2-1 / M2-3）—— 开发侧唯一还没被证据支撑的一块。
代码齐了，但机器上**没有任何 `ai_config.json`，一次都没真跑过**。
`agenttest` 是用 `patch_draft` 模拟 agent 的，不碰 `provider` / `channel_b`：
它证明「AI 改完之后那套链路对」，**不是「AI 能改」**。
最小一步：向用户要一个 DeepSeek key（通道 A），在应用里说一句
「把这个按钮改成 danger 态」，看它是不是真调了我们的工具并落盘。判据见 `doc/01` §7.6 第 23 条。
key 只放 `ai_config.json`，**不进仓库、不写进任何文档**。

**线 B · 和设计侧把这一轮走完**（见 §5）—— 把底稿送到 ClaudeDesign、收回它的稿、
过 `incoming`，然后实现 `doc/12` M5-8 / M5-9 / M5-10。
...
| 最新发件包 | `outgoing/UmbraDesign-ui-20260921-1456.zip`（打包后 ui/ 未改） |
| 包是否已送到 ClaudeDesign | **不确定** —— 先连上去看它项目里有没有 S6/S8/S9/S10、S1 里有没有 baseline 行。没有就按 5.2 送过去 |
| 本地 `ui/_incoming/` | 放的是**上一轮的废稿**（老底稿上改的 S1–S5、S7、IconGlyph、_ds-tool），已被 `.gitignore` 忽略。清掉再收新稿 |

### 5.4 这一轮要设计侧交回什么（细则在 `doc/14` §零 0.3，以那里为准）
...
## 9. 当前状态一句话（2026-09-22）

M0 / M1 已验收；M3 外壳能跑；**界面十屏全部可渲染、演示态可切**；
**两条 AI 通道代码在但从未真跑过**；设计侧这一轮**等底稿送达后重做**。进度 64 / 73。下一步见 §4。

⚠️ 先看一眼 `doc/00` §三十、§三十一 —— 曾有三屏被标成 ✅ 而实际渲染不出来
（校验器当时漏报），补了判据才暴露。
**别只信状态列，跑一次 `selftest` + `rendertest` 再说。**

待清理（未进仓库的残留，确认后可删）：`ui/_incoming/` 里的上一轮废稿、`outgoing/ziLBtpQN`（打包临时文件）、
`doc/_to_delete_wb9.sha`。
