...
**线 B · 设计侧第二轮已收完、M5-8 / M5-9 已接线并实测**（2026-09-23，`doc/00` §三十三）。
剩 `doc/12` M5-10：应用前端 UI-1..UI-8（`server/ui/index.html`）。
取 ClaudeDesign 文件的坑（预览链接带注入脚本，要先校准再剥）记在同一节。
...
M0 / M1 已验收；M3 外壳能跑；**界面十屏全部可渲染、演示态可切**；设计侧第二轮**已收完并入**，
S1 索引过期 / S2 版本弹层 / 未落盘横条**接真数据并实测**；**两条 AI 通道代码在但从未真跑过**。进度 66 / 73。下一步见 §4。
