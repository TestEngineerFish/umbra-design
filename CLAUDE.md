...
**三种编辑方式都在应用里闭环**（① S2 壳属性面板 · ② 选中节点带给 AI · ③ 直接对话），改稿 → 变更卡 → 一键回退；**S8 项目设置也进了应用**；会话**作业化：边跑边看工具行、真中断**（§四十）；壳内自测（§三十八）在 Tauri 壳里走通主流程并修掉三条壳里的缺陷。进度 70 / 74。仓库已上 GitHub（`origin` = `TestEngineerFish/umbra-design`）。对照 ClaudeDesign 的结论在 `doc/16`，待拍板 Q13–Q15。剩通道 B 等套餐续订。
