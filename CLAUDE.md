...
2. **M9-1 壳 spike**（半天，可与 M7 并行）：playwright-core 经 CDP 接 Electron 自带 Chromium 跑 `render_check`；主进程内跑核心不影响 MCP stdio。读数写进 `00`，回填 `11` Q26
3. **M7-9 设计侧第四轮**：`doc/14` 第四轮 + `doc/08` §三之三 已写好，`npm run outgoing` 打包后按 `claude-design-sync` 放进它的 `uploads/`
