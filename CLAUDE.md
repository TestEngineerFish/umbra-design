...
**先补三份渲染不出来的稿：`S6-版本对比` / `S9-会话面板` / `S10-组件Props面板`。**

它们现在既没引 `support.js`、也没有 `data-dc-script` 的逻辑类，
模板里却写了 98 / 38 / 49 个洞 —— 运行时不加载，`{{ … }}` 会原样印在画面上。
（来龙去脉见 `doc/00` §三十）

判据：`npm --prefix server run selftest` 的「工具界面稿 ui/」那一块回到 **零 error**。
现在会报 6 条（三份 × 2）。

**再验证两条 AI 通道**（`doc/12` M2-1 / M2-3）。代码齐了，但机器上
**没有任何 `ai_config.json`，一次都没真跑过**。注意 `agenttest` 是用
`patch_draft` 模拟 agent 的，不碰 `provider` / `channel_b` ——
它证明的是「AI 改完之后那套链路对」，不是「AI 能改」。
配一个 DeepSeek key 跑通通道 A，是把 M2 从「代码在」变成「能用」的最小一步。
...
## 8. 当前状态一句话（2026-09-21 复核）

M0 / M1 已验收；M3 外壳能跑；界面十屏有文件，但**只有七屏真能渲染**
（S6 / S9 / S10 是静态草图）；**两条 AI 通道代码在但从未真跑过**。
进度 59 / 70。下一步见 §4。

⚠️ 读进度表之前先看一眼 `doc/00` §三十 ——
上一轮有五条被标成 ✅ 而实际渲染不出来，复核后已退回 🚧。
**别只信状态列，跑一次 `selftest` 再说。**
