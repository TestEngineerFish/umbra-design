...
**M1 生命周期**，第一条是 **M1-0：引用图谱**（`doc/12` §二）。
...
M1-0 引用图谱（谁引用了我）
   → M1-1 create_project
   → M1-3 create_draft
   → M1-4/6/7 改名 / 移动 / 删除（都依赖 M1-0）
```

**为什么引用图谱必须最先**：`dc-import name="X"` 是**按文件名解析**的。
改名 / 移动 / 删除任何一份被引用的稿都会**静默打断引用**，
而 `E_IMPORT_MISSING` 要等下一次校验才报。不先解决它，后面每条都在埋雷。
（`doc/01` H4）

M1 的验收口径是 `doc/01` §7.5 第 18–20 条：
空目录起步，建 1 个项目 + 3 份稿（含一份被引用的组件），
改名一次、移动一次、删一次、回收站恢复一次，全程只用界面，
每步之后 `E_IMPORT_MISSING` 都是 0。
...
M0 已完成并验收（27 个工具、6 屏界面、三层回归全绿、编辑方式 ① 打通）。
**下一步 M1-0 引用图谱。** 编辑方式 ②③ 与应用外壳都还没开始。
