...
**进度 129 / 141。** 回归读数：`uitest` **115/115** · `selftest` 零 error · `lifecycletest` 全通 ·
...
3. **M8-17** 面板层解耦（`SidePanels` 还在按 panel id 分支，`PropsPanel` / `FileCard` 还在 `workbench/`）。
   评论指针那半边设计侧第九轮已裁决不做（「评论不是另一种指针，是选中之后的一个动作」）
4. **M9-6** win 真机第一次跑（要用户有 Windows 机器），然后 **M10**（第二批类型 / 秘书接入；Web 版不排期）
