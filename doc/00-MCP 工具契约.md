...
| # | 它问的 | 实测 / 我们的做法 |
| --- | --- | --- |
| 1 | ⌘E 会不会和稿内键盘冲突（焦点在稿里按 ⌘E 该不该透出来） | **不是冲突，是够不着**：稿在 iframe 里，焦点进去之后 keydown 根本到不了我们的 document。实测焦点在外 ⌘E 正常、焦点在稿里完全无反应。要它在稿里也生效，得 S2 壳监听 keydown 再 postMessage 转出来 —— **改的是它的稿，等它定** |
