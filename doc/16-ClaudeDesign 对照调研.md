# Umbra Studio · 16 ClaudeDesign 对照调研（2026-09-23）
...
| 能力 | ClaudeDesign | Umbra Studio 现状 | 差距 |
| --- | --- | --- | --- |
...
**J2 · 缺的是「评论」这个实体，不是通道。** 方式 ② 已经能指着元素说话，但那句话只活在会话里，不钉在元素上、不能列出来、不能标「已处理」。它的 Comment 模式是团队协作用的（Leave feedback for your teammates）；我们单机、单人，**判断**：先做「钉在节点上的便签」（存 `.umbrastudio/comments.json`，S2 里显示钉子，会话里能一键「把这条评论发给 AI」），不做画框。
